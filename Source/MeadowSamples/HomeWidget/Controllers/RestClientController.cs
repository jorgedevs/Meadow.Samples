...
public class RestClientController
{
    ...
    public async Task<OutdoorWeatherConditions> GetWeatherForecast()
    {
        using (HttpClient client = new HttpClient())
        {
            try
            {
                ...
                var sunrise = DateTimeOffset.FromUnixTimeSeconds(values.sys.sunrise).DateTime.AddHours(TIMEZONE_OFFSET);
                var sunset = DateTimeOffset.FromUnixTimeSeconds(values.sys.sunset).DateTime.AddHours(TIMEZONE_OFFSET);
                bool isDayLight = today > sunrise && today < sunset;
                string weatherIconFile = GetWeatherIcon(values.weather[0].id, isDayLight);

                var weatherConditions = new OutdoorWeatherConditions()
                {
                    weatherIcon = weatherIconFile,
                    temperature = values.main.temp - 273,
                    feelsLike = values.main.feels_like - 273,
                    humidity = values.main.humidity,
                    pressure = values.main.pressure,
                    sunrise = sunrise.ToString("h:mm tt"),
                    sunset = sunset.ToString("h:mm tt")
                };
